          ...
          <div class="row full-width justify-between">
            ...
            <div class="col-4 row justify-between q-pr-md">
              ...
              <div class="col column justify-center items-center text-center">
                <q-btn
                   v-if="account?.plan?.isTrail == false"
                   flat round color="primary" icon="mdi-download" no-caps
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const productData = computed(
      () => $store.getters["shopify/getShopifyFields"]
    );
    const defaultMapData = computed(
      () => $store.getters["shopify/defaultMapData"]
    );
    ...
    onBeforeMount(async () => {
    await $store.dispatch('shopify/getAnalytics')
    await $store.dispatch("shopify/getShopifyCredentials")
    isShopNamePresent.value = computed(() =>  $store.getters["shopify/getShopifyName"]);
    isShopKeyPresent.value = computed(() =>  $store.getters["shopify/getShopifyKey"]);
      let storedMapValues = await $store.dispatch("shopify/getMapData");
      selectedTime.value =
        storedMapValues.length > 0 ? storedMapValues[0].scheduleTime : "10:00";
      let getScheduleStatus = await $store.dispatch(
        "shopify/getScheduleStatus"
      );
      $store.dispatch("home/getCatalogDetails").then((suppliers) => {
        let data = $store.getters["shopify/getShopifyFields"];
        let defaultData = $store.getters["shopify/defaultMapData"];
        suppliers.forEach(async (supplier,ind) => {
          let isProgress = false;
          let isEnabled = false;
          let isInProgress = await $store.dispatch(
            "shopify/getProgressStatus",
    ...
    const downloadImports = async (supplier) => {
      ...
      await $store.dispatch("shopify/download", supplier);
      $q.loading.hide();
    };
    return {
      ...
      scheduleClick() {
        ...
        $store.dispatch("shopify/storeScheduleTime", data);
      },
      editMap(supplier) {
      ...
      async saveMapData() {
        await $store.dispatch("shopify/getShopifyCredentials")
        const isShopNamePresent = $store.getters["shopify/getShopifyName"];
        const isShopUrlPresent = $store.getters["shopify/getShopifyKey"];
        if(isShopNamePresent === "" && isShopUrlPresent === ""){
          $q.notify({
            type: "negative",
            position: "top",
            message: "Shop name and API secret key is required! ",
          });
          return
        }
        ...
        let savedata = $store.dispatch("shopify/saveAll", syncData.value);
        $q.notify({
      ...
      saveMap(supplier, IsAll) {
        ...
        let storedata = $store.dispatch("shopify/saveAll", syncData.value);
        $q.notify({
          spinner: QSpinnerGears,
          color: "primary",
      ...
      async scheduleSync(supplier) {
        ...
        let status = await $store.dispatch("shopify/saveAll", finalObject);
